...
- [x] **#3 Add a folder / git repo on the REMOTE daemon from local UI**
      ...
      directory browser** that REUSES the file tree's own building blocks
      (`FileTreeNode` with a new `hideActions` picker mode + a new
      `FilePathBar` breadcrumb extracted from FileBrowser's address bar and
      shared by both). It starts at the box's home (new `GET /api/home`,
      auto-proxied; e2e-tested), falls back to the parent of an existing repo
      on that daemon (so it works even on a remote predating `/api/home`),
      and lists subfolders via proxied `fetchDir`/`GET /api/files`; typing/
      paste + Enter still works. STILL TODO: remote `git clone`/`init` (no
      endpoint yet — repo must already be on the box).
