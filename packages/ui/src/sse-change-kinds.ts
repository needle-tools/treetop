 ...
 *  them is wasted work. */
export const CHANGE_KINDS_REQUIRING_EVENTS_RELOAD: ReadonlySet<string> = new Set([
...
export function changeKindRequiresReposReload(kind: unknown): boolean {
  return (
    typeof kind === "string" &&
    CHANGE_KINDS_REQUIRING_REPOS_RELOAD.has(kind)
  );
}
...
export function changeKindRequiresEventsReload(kind: unknown): boolean {
  return (
    typeof kind === "string" &&
    CHANGE_KINDS_REQUIRING_EVENTS_RELOAD.has(kind)
  );
}
