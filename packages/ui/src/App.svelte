...
<style>
  ...
  /* Modal (.modal-scrim, .modal, .modal-action variants) + toast stack
     (.toast-stack, .toast, .toast-icon, .toast-close, kind variants)
     moved to styles/overlays.css. */
  .branch.detached {
  ...
  /* .new-session-col + .new-session-head family (action buttons,
     awaiting-input pulse, manual-title editor) moved to
  ...
  /* .agents-popover (shell + anchoring) moved to styles/popover.css.
     .agents-list + .agent-row family (incl. brand variants, title,
     msgs/time/sid cells, row-close) moved to styles/agent-row.css. */
  .row-activity {
    display: flex;
    align-items: center;
    gap: 0.4rem;
  ...
  /* Source-control foldout family (.expanded + .inline-diff + tabs +
     ctx-toggle + .commits-heading + .commits + .commit-row +
     .inline-commit + .load-more) moved to styles/source-control.css. */
  ...
  /* Per-row zen-mode CSS (main.zen-row, .row.row-zen + child overrides,
     .hide-history-btn, .row-zen-btn) moved to styles/zen-row.css. */

  /* .new-session-col:fullscreen moved to styles/new-session.css. */
</style>
