...
<script lang="ts">
  ...
  function messageBaseStampId(value: NoteShape): number {
    const sessionAnchor = value.anchors.find((a) => a.startsWith("session:"));
    const seed = [
      value.sender?.id ?? value.sender?.source ?? sessionAnchor ?? value.id,
      value.receiver?.sessionId ?? value.receiver?.peerId ?? "",
  ...
  let optimisticStampNoteId: string | null = null;
  let optimisticStampId: number | null = null;

  $: if (optimisticStampNoteId && optimisticStampNoteId !== note.id) {
    optimisticStampNoteId = null;
    optimisticStampId = null;
  }

  $: if (
    optimisticStampNoteId === note.id &&
    optimisticStampId !== null &&
    note.stampId === optimisticStampId
  ) {
    optimisticStampNoteId = null;
    optimisticStampId = null;
  }

  function messageStampId(value: NoteShape, optimistic: number | null): number {
    return optimistic ?? value.stampId ?? messageBaseStampId(value);
  }

  function messageStampStyle(stampId: number): string {
  ...
  function messageStampRotation(stampId: number): number {
    const mixed = Math.imul(stampId ^ 0x85ebca6b, 0xc2b2ae35) >>> 0;
    return Math.round(((mixed % 1401) / 100 - 7) * 10) / 10;
  }
  ...
  function rerollMessageStamp(): void {
    const nextStampId = randomMessageStampId(
      messageStampId(note, optimisticStampId),
    );
    optimisticStampNoteId = note.id;
    optimisticStampId = nextStampId;
    dispatch("save", {
      id: note.id,
      body: note.body,
      stampId: nextStampId,
...
{#snippet messageEnvelope()}
  {@const delivery = messageDelivery()}
  {@const stampId = messageStampId(note, optimisticStampId)}
  {@const stampStyle = messageStampStyle(stampId)}
  {@const stampRotation = messageStampRotation(stampId)}
  <section
  ...
  >
    <div class="message-envelope-face" aria-hidden={messageOpen}>
      ...
      <button
        ...
        style:--message-stamp-rotation={`${stampRotation}deg`}
...
{#snippet attachmentPreview(
  ...
  {:else if attachment.kind === "emoji"}
    ...
    {@const attachedAppName = sticker ? null : appIconNameFromToken(attachment.body)}
    ...
    {:else}
      <span
        ...
        >{#if sticker !== null}<span
          ...
          ></span>{:else if attachedAppName}<img
