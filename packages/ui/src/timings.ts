/**
 * Rolling latency recorder for hot paths (UI edition).
 *
 * Records the last N=256 samples per named span and reports percentiles.
 * Sync, cheap, zero external dependencies. Browser-safe: uses
 * `performance.now()` which is available in every modern browser and in
 * Bun's test runtime.
 *
 * Public API is intentionally identical to packages/daemon/src/timings.ts
 * so the two sides can be read and reasoned about together.
 *
 * Usage:
 ...
 *
 *   // Devtools console:
 *   __sgTimings()  // only available in DEV mode
 */
...
if (typeof import.meta !== "undefined" && (import.meta as any).env?.DEV) {
  (globalThis as any).__sgTimings = snapshot;
}
