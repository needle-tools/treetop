<script lang="ts">
  ...
  $: isStarred =
    starred.has(fullPath) ||
    starred.has(fullPath.replace(/\//g, "\\")) ||
    starred.has(fullPath.replace(/\\/g, "/"));
  ...
  async function loadFileDiff() {
    ...
    try {
      ...
      if (!text && primary !== "workdir")
        text = await fetchDiffForKind(file, "workdir");
      if (!text && primary !== "staged")
        text = await fetchDiffForKind(file, "staged");
      if (!text && primary !== "untracked")
        text = await fetchDiffForKind(file, "untracked");
      diffText = text || "";
    } catch {
  ...
  async function loadStatDiff(filePath: string) {
    ...
    try {
      ...
      statDiffs = {
        ...statDiffs,
        [filePath]: { loading: false, text: text || "" },
      };
    } catch {
      statDiffs = {
        ...statDiffs,
        [filePath]: { loading: false, text: "(failed)" },
      };
    }
  }
...
<li>
  <div
    ...
    on:keydown={(e) => {
      if (e.key === "Enter") onSelect(fullPath, e);
    }}
  ...
  >
    {#if isDir}
      <span
        ...
        ><svg class="fb-tri" class:fb-tri-open={isExpanded} viewBox="0 0 8 8"
              ...
              {#each folderStats as f}
                ...
                >
                  <button
                    ...
                    on:click={() => {
                      const abs = joinPath(wtPath, f.path);
                      onNavigateToFile(abs);
                    }}
                  >
                    ...
                    {:else}
                      {#if f.added > 0}<span class="fb-stat-add"
                          >+{f.added}</span
                        >{/if}
                      {#if f.removed > 0}<span class="fb-stat-rm"
                          >-{f.removed}</span
                        >{/if}
                    {/if}
                  </button>
    ...
    {#if entry.sync}
      <span
        class="fb-sync fb-sync-{entry.sync}"
        title={entry.sync === "downloading"
          ? "Downloading…"
